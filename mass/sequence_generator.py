        ...
        **kwargs
        ...
        for step in range(max_len + 1):  # one extra step for EOS marker
            ...
            lprobs, avg_attn_scores = model.forward_decoder(
                tokens[:, :step + 1], encoder_outs=encoder_outs, temperature=self.temperature, lang_pair=lang_pair,
            )
...
class EnsembleModel(torch.nn.Module):
    ...
    @torch.no_grad()
    def forward_decoder(self, tokens, encoder_outs, temperature=1., lang_pair=None):
    ...
    def _decode_one(
        ...
        temperature=1., lang_pair=None,
    ):
        if self.incremental_states is not None:
            decoder_out = list(model.forward_decoder(
                tokens, encoder_out=encoder_out, incremental_state=self.incremental_states[model], lang_pair=lang_pair,
            ))
        else:
            decoder_out = list(model.forward_decoder(tokens, encoder_out=encoder_out, lang_pair=lang_pair,))
